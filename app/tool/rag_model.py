...
class RagSearch(BaseTool):
    ...
    async def _should_store_new_content(self, existing_content: str, new_content: str) -> bool:
        ...
        try:
            ...
            system_msg = Message.system_message(
                """Sina old füüsika õpetaja abiline ja aitad teda tema töödes."""
            )
            
            user_msg = Message.user_message(
                f"""Kas peaksin salvestama uue sisu, võttes arvesse järgmist infot?
            ...
            return response.strip().lower() == 'jah'
            
        except Exception as e:
            print(f"Error comparing content: {e}")
            return True  # Default to storing new content if comparison fails
    ...
    async def execute(self, **kwargs) -> list:
        ...
        try:
            ...
            await self._store_result(session_id, step_index, formatted_content)
